...
function NavItems() {
  ...
  const [notifList, setNotifList] = useState(localStorage.getItem('notifs') || []);
  const [friendList, setFriendList] = useState(localStorage.getItem('notifs') || []);
  ...
  useLayoutEffect(() => {
    // fetch notifList and friendList
    setNotifList(['Notif1', 'Notif2']);
    setFriendList(['Friend1', 'Friend2', 'Friend3']);
  }, []);

  useEffect(() => {
    if (chosenItem === 'notifs') {
      setItemTitle('Notifications');
      setChosenList([...notifList]);
    } else if (chosenItem === 'friends') {
      setItemTitle('Friend Requests');
      setChosenList([...friendList]);
    }
  }, [chosenItem]);

  return (
    <div className="flex flex-row w-full justify-end items-center space-x-3 dropdown dropdown-bottom dropdown-end">
      <div tabIndex={0} onClick={() => {chosenItem === 'notifs' && setDropdownOpen(prev => !prev);setChosenItem('notifs')}} className="space-y-0.5">
      ...
      <div tabIndex={0} onClick={() => {chosenItem === 'friends' && setDropdownOpen(prev => !prev);setChosenItem('friends')}} className="space-y-0.5">
      ...
      <div tabIndex={0} className="dropdown-content menu bg-base-100 rounded-box z-[1] w-full mt-2 p-2">
        <h2>{itemTitle}</h2>
        {chosenList ? <ul>{chosenList.map(item => <li>{item}</li>)}</ul> : <p>Sorry. It's empty</p>}
      </div>
    </div>
  )
}
